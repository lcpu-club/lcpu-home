...
const categories: { title: string, route: string, pageGroups: { year: number; month: number; items: PageData[]; }[] }[] = []

Object.keys(RouteTitleRecord).forEach(category => {
  categories.push({
    title: SiteConfiguration.getRouteCategoryTitle(category),
    route: `/${category}/`,
    pageGroups: groupByYearMonth(allPages.filter(page => page.category === category).sort((a, b) => Date.parse(b.time) - Date.parse(a.time))),
  })
})
...
<template>
  <div w-screen h-screen lg:w-unset box-border fixed lg:position-unset h-full z-999 transition-all duration-300
    @click="toggleSidebar(true)" pointer-events-none lg:pointer-events-unset class="h-100dvh!"
    ...
    <div @click.stop h-full box-border p-x-6 p-y-12 md:p-x-12 bg-gray-100 dark:bg-dark-800 transition-all duration-300
      ...
      :class="{ 'translate-x-0! shadow-xl': !sidebarCollapsed }" lg:shadow-none>
      ...
      <ExpanderComponent m-t-4 v-for="category in categories" :key="category.title">
        <template #header>
          <h3 m-0>
            <a :href="category.route" class="text-unset!" decoration-none @click="toggleSidebar()">{{ category.title
              }}</a>
          </h3>
        </template>
        <div flex="~ col" box-border>
          <div v-for="pageGroup in category.pageGroups" :key="pageGroup.year + '-' + pageGroup.month" flex="~ col gap-2"
            class="group" border-t-1 border-gray-200 dark:border-dark-200 border-t-solid p-y-3>
            <span text-xs text-gray-500 dark:text-truegray-400>{{ pageGroup.year }} 年 {{ pageGroup.month }} 月
            </span>
            <a v-for="page in pageGroup.items" @click="toggleSidebar()" :href="page.contentUrl" text-wrap
              :key="page.title"
