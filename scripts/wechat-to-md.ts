...
function extractArticleContent(html: string): { content: string; defaultTitle: string; publishTime: string } {
  const dom = new JSDOM(html);
  const doc = dom.window.document;

  // 提取标题
  const defaultTitle = doc.querySelector('#activity-name')?.textContent?.trim()
